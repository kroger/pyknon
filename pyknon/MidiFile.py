...
class MIDIEvent:
    ...
    def __eq__(self, other):
        return self.time == other.time or self.ord == other.ord
...
class GenericEvent:
    ...
    def __hash__(self):
        ...
        return hashlib.sha1(str(self.time).encode()).hexdigest()
...
class tempo(GenericEvent):
    ...
    def __eq__(self, other):
        return super(tempo, self).__eq__(other) and self.tempo == other.tempo
...
class programChange(GenericEvent):
    ...
    def __eq__(self, other):
        return super(programChange, self).__eq__(other) and \
...
class SysExEvent(GenericEvent):
    ...
    def __eq__(self, other):
        return super(SysExEvent, self).__eq__(other) and self.manID == other.manID
...
class UniversalSysExEvent(GenericEvent):
    ...
    def __eq__(self, other):
        return super(UniversalSysExEvent, self).__eq__(other) and \
            self.code == other.code and \
            self.subcode == other.subcode and \
...
class ControllerEvent(GenericEvent):
    ...
    def __eq__(self, other):
        return super(ControllerEvent, self).__eq__(other) and \
            self.parameter1 == other.parameter1 and \
...
class trackName(GenericEvent):
    ...
    def __eq__(self, other):
        return super(trackName, self).__eq__(other) and \
...
class MIDITrack:
    ...
    def addUniversalSysEx(self, time, code, subcode, payload, sysExChannel=0x7F,
        ...
        self.eventList.append(UniversalSysExEvent(
            time, realTime, sysExChannel, code, subcode, payload))
    ...
    def changeNoteTuning(self, tunings, sysExChannel=0x7F, realTime=False, tuningProgam=0):
        ...
        self.eventList.append(UniversalSysExEvent(
            0, realTime, sysExChannel, 8, 2, payload))
...
class MIDIFile:
    ...
    def __init__(self, numTracks, removeDuplicates=True, deinterleave=True):
        ...
        self.tracks = [MIDITrack(removeDuplicates, deinterleave) for _ in numTracks]
        self.closed = False

    # Public Functions. These (for the most part) wrap the MIDITrack functions, where most
    # Processing takes plac e.
    ...
    def addTempo(self, track, time, tempo):
        ...
        Use:
            MyMIDI.addTempo(track, time, temp o )

        Arguments:
             track: The track to which the event is added. [Integer, 0-127].
            time: The time at which the event is added, in beats. [Float].            tempo: The tempo, in Beats per Minute. [Integer]
        """
    ...
    def shiftTracks(self, offset=0):
        ...
        for track in self.tracks:
            for event in track.eventList:
                event.time = event.time - origin + offset
    ...
    def close(self):
        ...
        for track in self.tracks:
            ...
            track.MIDIEventList.sort()

        origin = self.findOrigin()

        for track in self.tracks:
            track.adjustTime(origin)
            track.writeMIDIStream()
    ...
    # TODO: -- Consider making this less efficient but more robust by not assuming the list to be sorted.
        ...
        for track in self.tracks:
            if track.MIDIEventList and track.MIDIEventList[0].time < origin:
